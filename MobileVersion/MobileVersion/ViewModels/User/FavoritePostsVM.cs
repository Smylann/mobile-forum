...
{
    public partial class FavoritePostsVM : ViewModelBase
    {
        ...
        public ObservableCollection<Homepage.PostVM>? FavoritePosts { get; } = new();

        public FavoritePostsVM(consoleClientModel model, UserVM user, string username, IEnumerable<Favorites> posts)
        {
            _model = model;
            _userprofile = user;
            Username = username;
            _fullPosts = posts.Select(p => new Homepage.PostVM(_model, p, null,null, null, null, this) ).ToList();
            UpdateVisiblePosts();
        }
        [RelayCommand]
        ...
        {
            var toAdd = _fullPosts.Skip(FavoritePosts.Count).Take(DisplayLimit - FavoritePosts.Count); //we skip the already loaded ones, and add 10 more
